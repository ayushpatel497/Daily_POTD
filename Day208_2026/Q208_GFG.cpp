...
class Solution {
    ...
    Node* build(vector<int>& pre, vector<int>& mirror, int ps, int pe, int ms, int me) {
        ...
        root-> left = build(pre, mirror, ps + 1, ps + leftSize, mirrorPos[leftRoot], me);
        root-> right = build(pre, mirror, ps + leftSize + 1, pe, ms + 1, mirrorPos[rightRoot]);

        return root;
    }
  public:
    Node *constructBinaryTree(vector<int> &pre, vector<int> &preMirror) {
        // Q. Construct a Full Binary Tree
        // code here
        int n = pre.size();
        for (int i = 0; i < n; i++)
            prePos[pre[i]] = i;
