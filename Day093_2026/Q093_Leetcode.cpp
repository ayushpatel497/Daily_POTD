...
public:
    int maxWalls(vector<int>& robots, vector<int>& distance, vector<int>& walls) {
        // Q. Maximum Walls Destroyed by Robots
        int n = robots.size();
        ...
        for (int i = 0; i < n; i++) {
            ...
            int leftBound = (i >= 1) ? max(robotPos - robotDistVal, robotDist[i - 1].first + 1)
            ...
            int rightBound = (i < n - 1) ? min(robotPos + robotDistVal, robotDist[i + 1].first - 1)
                                         : robotPos + robotDistVal;
