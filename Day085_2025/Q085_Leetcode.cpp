        ...
        int minOperations(vector<vector<int>>& grid, int x) {
            ...
            while (prefixIndex < suffixIndex) {
                if (prefixIndex < length - suffixIndex - 1) {
                    int prefixOperations =(prefixIndex + 1) * (numsArray[prefixIndex + 1] - numsArray[prefixIndex]) / x;
                    result += prefixOperations;
                    prefixIndex++;
                } else {
                    int suffixOperations = (length - suffixIndex) * (numsArray[suffixIndex] - numsArray[suffixIndex - 1]) / x;
